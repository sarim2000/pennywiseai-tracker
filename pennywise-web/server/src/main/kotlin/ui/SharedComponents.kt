...
object SharedComponents {
        ...
        div(classes = "header") {
            div(classes = "header-content") {
                ...
                nav(classes = "nav-links") {
                    ...
                    a(href = "/pro", classes = if (currentPage == "pro") "nav-pro active" else "nav-pro") {
                        +"Get Pro"
                    }
    ...
    /**
     * The one loud element on the site. Play can't sell Pro right now and the app
     * isn't allowed to link here, so the landing page has to make the offer
     * unmissable: prices up front, one tap to /pro.
     */
    fun FlowContent.proBanner() {
        a(href = "/pro", classes = "pro-banner") {
            span(classes = "pro-banner-title") { +"PennyWise Pro" }
            span(classes = "pro-banner-prices") { +"₹2,000 lifetime · ₹699 / year · ₹149 / month" }
            span(classes = "pro-banner-cta") { +"Get Pro →" }
        }
    }
    ...
    val commonStyles = """
        ...
        * { box-sizing: border-box; }
        ...
        .nav-links a.nav-pro { color: var(--ink); background: var(--amber); padding: 6px 12px; border-radius: 999px; font-weight: 700; }
        .nav-links a.nav-pro:hover { background: #FFC875; color: var(--ink); }
        .pro-banner { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; margin: 0 0 22px; padding: 14px 18px; border-radius: 14px; text-decoration: none; color: var(--ink); background: linear-gradient(100deg, var(--amber), #FFD08A); border: 1px solid rgba(255,183,77,.6); transition: transform .12s, box-shadow .15s; }
        .pro-banner:hover { transform: translateY(-1px); box-shadow: 0 8px 28px rgba(255,183,77,.25); }
        .pro-banner-title { font-family: var(--display); font-weight: 800; font-size: 17px; letter-spacing: -.02em; }
        .pro-banner-prices { font-family: var(--mono); font-size: 13px; opacity: .85; flex: 1; }
        .pro-banner-cta { font-weight: 700; font-size: 14px; white-space: nowrap; }
        ...
        @media (max-width: 640px) {
          ...
          .pro-banner { gap: 6px; }
          .pro-banner-prices { flex-basis: 100%; }
        }
