...
object LicensePolicy {
    ...
    /** True when the last successful validation is old enough to re-check. */
    fun isDue(lastValidatedAt: Long, now: Long): Boolean = now - lastValidatedAt >= REVALIDATE_AFTER_MS

    /** True while the stored license still grants Pro without a fresh check. */
    fun grantsPro(lastValidatedAt: Long, now: Long): Boolean =
        lastValidatedAt > 0 && now - lastValidatedAt < EXPIRE_AFTER_MS
}
